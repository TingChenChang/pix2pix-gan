...
for epoch in range(1, NUM_EPOCHS + 1):
    ...
    torch.save(generator.state_dict(), f'{MODEL_DIR}/generator_epoch_{epoch}.pth')
    torch.save(discriminator.state_dict(), f'{MODEL_DIR}/discriminator_epoch_{epoch}.pth')
    
    for (inputs, targets), _ in val_dl:
        inputs = inputs.to(device)
        generated_output = generator(inputs)
        save_image(generated_output.data[:10], f'{MODEL_DIR}/images/sample_{epoch}.png', nrow=5, normalize=True)
